class LastWordLength {
    /*
     * Given a string s consists of upper/lower-case alphabets and 
     * empty space characters ' ', return the length of last word in 
     * the string. If the last word does not exist, return 0.
     * Note: A word is defined as a character sequence consists of 
     * non-space characters only.
     * For example, 
     * Given s = "Hello World",
     * return 5.
     */
    public static void main(String[] args) {
	System.out.println(len(args[0]));
        System.out.println(len2(args[0]));

    }
    private static int len(String in) {
	if (in == null) return 0;
	int len = 0;
	for (int i = in.length()-1; i >= 0; i--){
	    if (in.charAt(i) == ' ') {
		break;
	    } else {
		len++;
	    }
	}
	return len;
    }
    private static int len2(String in) {
	return in.split(" ")[in.split(" ").length-1].length();
    }
}